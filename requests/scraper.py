...
class Spider(object):
    ...
    def imgs(self, url):
        ...
        while re.match('https?://\w+[\.\w+]+([/\w+]+\.html?)?', url):
            ...
            if self.filename(url) != nexturl:
                url=url[:url.rfind('/')+1]+nexturl
            else:
                all_a=soup.find_all('a')
                for a in all_a:
                    if a.has_attr('href') and re.match('[/\w+]+/\d.(htm|html)',a['href']) and a['href'] not in url:
                        if self.host(url)+a['href'] not in history:
                            url=self.host(url)+a['href']
                            break
                        else:
                            continue
    ...
    def eq(self, url1, url2):
        return self.group(url1) == self.group(url2)
...
spider.imgs('https://www.4493.com/tiyumeinv/118592/1.htm')
